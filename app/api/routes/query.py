...
def query_rag(request: QueryRequest, db: Session = Depends(get_db)) -> Dict[str, Any]:
    ...
    try:
        ...
        retrieved_results = search_chunks(db=db, query_embedding=query_vector, top_k=request.top_k * 4)

        # Rerank ke top_k yang diminta
        from app.retrieval.reranker import rerank
        retrieved_results = rerank(query=request.question, chunks=retrieved_results, top_k=request.top_k)
        ...
        generated_answer = generate_answer(question=request.question, results=retrieved_results)
